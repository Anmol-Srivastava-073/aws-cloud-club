...
export default function EventCard({ event, index }: { event: Event; index: number }) {
  ...
  // 🔥 CARD CONTENT (reuse for both cases)
  const cardContent = (
    <motion.div
      initial={{ opacity: 0, y: 30 }}
      whileInView={{ opacity: 1, y: 0 }}
      whileHover={!expired ? { scale: 1.04, y: -4 } : {}}
      viewport={{ once: true }}
      transition={{ delay: index * 0.1 }}
      className={`group relative rounded-2xl p-[1px] bg-gradient-to-br from-indigo-500 via-purple-500 to-blue-500 bg-[length:200%_200%] animate-gradient h-full ${expired ? 'opacity-60' : ''}`}
    >
      <div className="rounded-2xl bg-[#0b0f1a]/90 backdrop-blur-xl p-8 border border-white/10 transition-all duration-300 group-hover:shadow-[0_0_50px_rgba(99,102,241,0.4)] flex flex-col h-full min-h-[280px]">
        ...
        {/* 🔥 Past Badge */}
        {expired && (
          <span className="text-xs px-2 py-1 rounded-full bg-red-500/20 text-red-400 mb-2 inline-block w-fit">
            Past Event
          </span>
        )}
        ...
        <div className="mt-auto flex items-center space-x-2 text-indigo-400 font-semibold group/btn">
          <span>{expired ? "Event Completed" : "View Details"}</span>
          {!expired && (
